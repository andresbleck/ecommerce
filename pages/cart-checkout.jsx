// Cart page + Checkout page (WhatsApp order flow)
    ...
    <Section tone="cream">
      ...
      <div className="lg-cartpage">
        ...
        <aside className="lg-summary">
          ...
          {shipping > 0 && (
            <div className="lg-summary__hint">Sumá {formatPrice(60000 - cart.total)} más y el envío es gratis.</div>
          )}
          <div className="lg-summary__sep"/>
          <div className="lg-summary__row lg-summary__row--big">
            <span>Total</span>
...
function CheckoutPage() {
  ...
  const sendOrder = async () => {
    const { error } = await sb.from('orders').insert({
      ...
      shipping_address: form.entrega === 'envio' ? {
        address: form.direccion, city: form.ciudad, postal_code: form.cp, province: form.provincia,
      } : null,
  ...
  return (
    <Section tone="cream">
      ...
      <p className="lg-lead" style={{marginBottom:32}}>Completá tus datos y mandamos el pedido por WhatsApp. Te respondemos con la forma de pago y el envío en minutos.</p>
      ...
      <div className="lg-checkout">
        <div className="lg-checkout__main">
          {step === 1 && (
            <form className="lg-form" onSubmit={(e) => { e.preventDefault(); setStep(2); }}>
              <fieldset className="lg-fs">
              ...
              <fieldset className="lg-fs" style={{marginTop:20}}>
                ...
                <div className="lg-pay">
                  <label className={'lg-pay__opt' + (form.entrega === 'envio' ? ' is-active' : '')}>
                    ...
                    <div>
                      <div className="lg-pay__l">Envío a domicilio</div>
                      <div className="lg-pay__s">OCA o Andreani · 2 a 5 días hábiles · {cart.total > 60000 ? 'gratis' : formatPrice(4500)}</div>
                    </div>
                  ...
                  <label className={'lg-pay__opt' + (form.entrega === 'retiro' ? ' is-active' : '')}>
                    ...
                    <div>
                      <div className="lg-pay__l">Retiro en taller</div>
                      <div className="lg-pay__s">Salta · sábados de 10 a 14h · sin costo</div>
                    </div>
                    <span className="lg-pay__radio"/>
                  </label>
                </div>
                ...
                <div className="lg-pay">
                  {[
                    ...
                    <label key={opt.id} className={'lg-pay__opt' + (form.pago === opt.id ? ' is-active' : '')}>
                      ...
                      <div>
                        <div className="lg-pay__l">{opt.label}</div>
                        <div className="lg-pay__s">{opt.sub}</div>
                      </div>
                      <span className="lg-pay__radio"/>
                    </label>
                  ))}
          ...
          {step === 2 && (
            <div className="lg-fs">
              <div className="lg-review-head">
                <div>
                  <legend style={{fontFamily:'var(--serif)', fontSize:26, marginBottom:6}}>Listo para enviar</legend>
              ...
              <div className="lg-wa-disclaimer">
                Se abrirá WhatsApp en una pestaña nueva. Si no te abre, podés escribirnos directo al <a href={`https://wa.me/${window.WHATSAPP_NUMBER}`} target="_blank">+54 9 11 0000-0000</a>.
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="lg-done">
              <div className="lg-done__check" style={{color:'#25D366'}}>
              ...
              <p className="lg-lead">Te respondemos por WhatsApp en los próximos minutos con la forma de pago y el detalle del envío. Si no te abrió la app, podés volver a enviarlo desde acá:</p>
              <div className="lg-form__actions lg-form__actions--center" style={{flexWrap:'wrap'}}>
              ...
              <div className="lg-done__num" style={{marginTop:32}}>Mientras tanto, cebá uno 🧉</div>
            </div>
          )}
        </div>
        ...
        {step < 3 && (
          <aside className="lg-summary">
            ...
            <ul className="lg-summary__list">
              {cart.items.map(it => (
                <li key={it.id}>
                  <div className="lg-drawer__thumb" style={{ background: it.accent }}><span>{it.monogram}</span></div>
                  <div>
                    <div className="lg-summary__name">{it.name}</div>
                    <div className="lg-summary__qty">x{it.qty}</div>
                  </div>
            ...
            <div className="lg-summary__safe" style={{marginTop:16}}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><path d="M12 7v5l3 2"/></svg>
              Coordinamos pago al confirmar
            </div>
          </aside>
        )}
      </div>
